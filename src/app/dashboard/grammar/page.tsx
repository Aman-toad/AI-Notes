...
import { ArrowLeft, Clipboard, ClipboardCheck, SpellCheck } from "lucide-react"
...
export default function GrammarPage() {
  ...
  const handleCopy = async () => {
    if (!correct) return
    await navigator.clipboard.writeText(correct)
    setCopied(true)
  ...
  return (
    <main className="relative min-h-[calc(100dvh-0px)]">
      ...
      <div className="relative z-10 p-6 max-w-3xl mx-auto">
        ...
        <h1 className="text-2xl font-semibold text-white mb-4">Grammar Checker</h1>
        ...
        {correct && (
          <Card className="mt-4 rounded-2xl border-white/10 bg-white/5 backdrop-blur-md p-4">
            ...
            <p className="text-slate-200">{correct}</p>
          </Card>
        )}
      </div>
    </main>
